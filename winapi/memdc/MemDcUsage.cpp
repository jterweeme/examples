...
static const double k_percentWidth = 0.33;
static const int k_rectCount = 10;

static const COLORREF k_gray                   = RGB(0xED, 0xED, 0xED);
static const COLORREF k_black                  = RGB(0x00, 0x00, 0x00);
static const COLORREF k_codeProjectDarkOrange  = RGB(0xFF, 0x99, 0x00);
static const COLORREF k_codeProjectLightOrange = RGB(0xFF, 0xE2, 0xA8);
static const COLORREF k_codeProjectDarkGreen   = RGB(0x48, 0x8E, 0x00);
static const COLORREF k_codeProjectLightGreen  = RGB(0x76, 0xAB, 0x40);

static const char k_instructions[]     = " -  Hold Left Mouse Button down on window to pause screen updates.\n"
...
static bool        g_useBackBuffer = true;   // Indicates if the back buffer is used or not.
static HDC         g_hMemDc      = NULL;     // Memory DC for the back buffer.
static HBITMAP     g_hBackBuffer = NULL;     // Back buffer bitmap.

static int         g_cur         = 0;        // Current offset from the beginning of the
                                      // animation.

static ULONGLONG   g_lastTime    = 0;        // Time marker for use in FPS calculations.
static UINT        g_frameCount  = 0;        // Current frame count for this second.
static UINT        g_desiredRate = 75;       // Track the desired frame-rate.
static UINT        g_desiredLen  = 5;        // The desired length in seconds of the animation.
static UINT        g_steps       = g_desiredRate * g_desiredLen; // count of steps in the sequence

static double      g_currentFps  = 0.0;      // Cache the last calculated FPS until
                                      // the next rate is calculated.

static RectList    g_group1;                 // list of random rectangles
static RectList    g_group2;                 // to make the display a bit more complicated.
static bool        g_isGroup1Active = true;  // Status for which group paints as ellipses.

static HPEN g_hPenBlack      = NULL;
static HBRUSH g_hBrGray        = NULL;
static HBRUSH g_hBrLightOrange = NULL;
static HBRUSH g_hBrDarkOrange  = NULL;
static HBRUSH g_hBrLightGreen  = NULL;
static HBRUSH g_hBrDarkGreen   = NULL;
static HBITMAP g_hWiperMask     = NULL;
...
Parameters: client[in]: The bounding rectangle for the entire animation.
            ...
            right[out]: A reference to the region handle to accept the right side.
              The caller is responsible to free this resource.
*******************************************************************************/
static void CreateAnimationClipRgns(const RECT &client,
    const RECT &rc, HRGN &hLeftRgn, HRGN &hRightRgn)
{
...
******************************************************************************/
template <int T> struct VertexPopulator
{
    void operator()(TRIVERTEX v[T], POINT pts[T], bool isOpaque[T], COLORREF c1, COLORREF c2)
    {
        for (int i = 0; i < T; ++i)
        {
            COLORREF color = isOpaque[i] ? c1 : c2;
            USHORT mask = isOpaque[i] ? 0xFF00 : 0x0000;
            v[i].x      = pts[i].x;
            v[i].y      = pts[i].y;
...
******************************************************************************/
static HBITMAP GetWiperAlphaBlendMask(HDC hDC, const RECT &rc, COLORREF color1, COLORREF)
{
...
Parameters: hDC[in]:
            ...
            hiColor[in]:
******************************************************************************/
static void DrawWiper(HDC hDC, const RECT &rc, COLORREF baseColor, COLORREF hiColor)
{
    ...
    ::AlphaBlend(hDC, rc.left, rc.top, bmpWidth, bmpHeight, hDCMem,
                0, 0, bmpWidth, bmpHeight, bFn);
...
*******************************************************************************/
static void DrawShapes(HDC hDC, HRGN hClipRgn, double width, double height,
                    RectList &list, bool isEllipse)
{
    int ctx = ::SaveDC(hDC);
    ...
    for (; cur != end; ++cur)
    {
        ...
        if (isEllipse)
        {
            ::Ellipse(hDC, (int)(cur->x * width), (int)(cur->y * height), (int)(cur->cx* width), (int)(cur->cy* height));
        }
        else
        {
            ::Rectangle(hDC, (int)(cur->x * width), (int)(cur->y * height), (int)(cur->cx* width), (int)(cur->cy* height));
        }
    }
...
*******************************************************************************/
static void GetRandomRectangle(const UINT width, const UINT height, RectInfo &randomRect)
{
...
Parameters: width[in]: Every generated rectangle will be smaller and fit
            ...
            randomRect[out]: The random rectangle output.
*******************************************************************************/
static void GetRectangleGroup(const UINT width, const UINT height, const UINT count, RectList &list)
{
...
static void UpdateFpsDisplay()
{
    ...
    {
        double diff = static_cast<double>(now - g_lastTime) / 1000.0;
        g_currentFps = static_cast<double>(g_frameCount) / diff;
        g_frameCount = 0;
        g_lastTime = now;
    }
}
...
int AdjustAnimation(HWND hWnd, int offset)
{
    ...
    const int hiWidth = static_cast<int>(width * k_percentWidth);
    const int aniRange = width + hiWidth;
    const int stepWidth = aniRange / g_steps;

    // Calculate the number of steps to offset.
    const int offsetCount = offset / stepWidth;
    g_cur += offsetCount;
...
void PaintAnimation(HWND hWnd, HDC hDC)
{
    ...
    RECT rc = { offsetX, client.top, static_cast<LONG>(offsetX + hiWidth), client.bottom };
