...
Element::Element(Element *parent, uintptr_t id, int x, int y, int width, int height)
  :
...
LRESULT Element::sendMsg(UINT msg, WPARAM wp, LPARAM lp) const
{
...
void Element::create(DWORD dwExStyle, LPCTSTR className, LPCTSTR windowName,
    ...
    HINSTANCE hInstance, LPVOID param)
{
    ...
    if (_hwnd == 0)
        throw TEXT("Cannot create element");
}
