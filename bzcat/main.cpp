...
private:
    ...
    uint32_t _nextByte();
    uint32_t _nextSymbol(BitInputBase *bi, const std::vector<Table> &t, const Fugt &selectors);
public:
    void reset();
    Block() { reset(); }
...
uint32_t Block::_nextSymbol(BitInputBase *bi, const std::vector<Table> &t, const Fugt &selectors)
{
    if (++_grpPos % 50 == 0)
