...
public:
    ...
    uint8_t indexToFront(uint8_t i)
    {
        uint8_t value = _buf[i];
...
static void decompress(std::istream &is, std::ostream &os, std::ostream &msg)
{
    ...
    while (true)
    {
        ...
        if (marker1 == 0x177245 && marker2 == 0x385090)
        {
            ...
            Toolbox t;
            msg << "0x" << t.hex32(crc) << " 0x" << t.hex32(streamCRC) << "\r\n";
            break;
        }
...
int main(int argc, char **argv)
{
    ...
    decompress(*is, std::cout, std::cerr);
    ifs.close();
    return 0;
}
