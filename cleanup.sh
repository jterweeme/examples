#!/bin/bash
rm -Rvf .vs
rm -Rvf Debug
rm -Rvf examples.ncb
rm -Rvf examples.pro.user
rm -Rvf examples.suo
rm -Rvf Release

rm -Rvf bgcolor/bgcolor.pro.user
rm -Rvf bgcolor/bgcolor.vcproj.DESKTOP-LGKFQ39.jasper.user
rm -Rvf bgcolor/Debug
rm -Rvf bgcolor/Release
rm -Rvf bgcolor/STANDARDSDK_500\ \(x86\)
rm -Rvf bgcolor/Win32
rm -Rvf bgcolor/x64

rm -Rvf bitmap/bitmap.pro.user
rm -Rvf bitmap/bitmap.vcproj.DESKTOP-LGKFQ39.jasper.user
rm -Rvf bitmap/Debug
rm -Rvf bitmap/Release
rm -Rvf bitmap/STANDARDSDK_500\ \(x86\)
rm -Rvf bitmap/Win32
rm -Rvf bitmap/x64

rm -Rvf cat/cat.pro.user
rm -Rvf cat/cat.vcproj.DESKTOP-LGKFQ39.jasper.user
rm -Rvf cat/Debug
rm -Rvf cat/Release
rm -Rvf cat/STANDARDSDK_500\ \(x86\)
rm -Rvf cat/Win32
rm -Rvf cat/x64

rm -Rvf cechat/cechat.pro.user
rm -Rvf cechat/cechat.vcproj.DESKTOP-LGKFQ39.jasper.user
rm -Rvf cechat/Debug
rm -Rvf cechat/Release
rm -Rvf cechat/STANDARDSDK_500\ \(x86\)
rm -Rvf cechat/Win32
rm -Rvf cechat/x64

rm -Rvf chess1/chess1.pro.user
rm -Rvf chess1/chess1.vcproj.DESKTOP-LGKFQ39.jasper.user
rm -Rvf chess1/Debug
rm -Rvf chess1/Release
rm -Rvf chess1/resource.aps
rm -Rvf chess1/STANDARDSDK_500\ \(x86\)/
rm -Rvf chess1/Win32
rm -Rvf chess1/x64

rm -Rvf cmdbar/cmdbar.vcproj.DESKTOP-LGKFQ39.jasper.user
rm -Rvf cmdbar/Debug
rm -Rvf cmdbar/Release
rm -Rvf cmdbar/STANDARDSDK_500\ \(x86\)
rm -Rvf cmdbar/x64

rm -Rvf contimeout/contimeout.vcproj.DESKTOP-LGKFQ39.jasper.user
rm -Rvf contimeout/STANDARDSDK_500\ \(x86\)

rm -Rvf md5sum/Debug
rm -Rvf md5sum/Release
rm -Rvf md5sum/md5sum.vcproj.DESKTOP-LGKFQ39.jasper.user
rm -Rvf md5sum/md5sum.pro.user
rm -Rvf md5sum/STANDARDSDK_500\ \(x86\)
rm -Rvf md5sum/Win32
rm -Rvf md5sum/x64

rm -Rvf netkatd/Debug
rm -Rvf netkatd/netkatd.vcproj.DESKTOP-LGKFQ39.jasper.user
rm -Rvf netkatd/Release
rm -Rvf netkatd/STANDARDSDK_500\ \(x86\)

rm -Rvf od/Debug
rm -Rvf od/od.vcproj.DESKTOP-LGKFQ39.jasper.user
rm -Rvf od/Release
rm -Rvf od/STANDARDSDK_500\ \(x86\)

rm -Rvf qrcode/Debug
rm -Rvf qrcode/Release
rm -Rvf qrcode/STANDARDSDK_500\ \(x86\)
rm -Rvf qrcode/qrcode.vcproj.DESKTOP-LGKFQ39.jasper.user
rm -Rvf qrcode/x64

rm -Rvf rx/fuzzymain.exe
rm -Rvf rx/rx
rm -Rvf rx/xrecv.log
rm -Rvf rx/.qmake.stash
rm -Rvf rx/*.o
rm -Rvf rx/Makefile
rm -Rvf rx/rx.vcproj.DESKTOP-LGKFQ39.jasper.user
rm -Rvf rx/Win32

rm -Rvf tftpd/Debug
rm -Rvf tftpd/Release
rm -Rvf tftpd/STANDARDSDK_500\ \(x86\)
rm -Rvf tftpd/tftpd.suo
rm -Rvf tftpd/tftpd.ncb
rm -Rvf tftpd/tftpd.vcproj.DESKTOP-LGKFQ39.jasper.user

rm -Rvf tictac/Debug
rm -Rvf tictac/Release
rm -Rvf tictac/STANDARDSDK_500\ \(x86\)
rm -Rvf tictac/tictac.vcproj.DESKTOP-LGKFQ39.jasper.user
rm -Rvf tictac/x64

rm -Rvf ws2client/Debug
rm -Rvf ws2client/Release
rm -Rvf ws2client/STANDARDSDK_500\ \(x86\)
rm -Rvf ws2client/Win32
rm -Rvf ws2client/ws2client.vcproj.DESKTOP-LGKFQ39.jasper.user

rm -Rvf ws2server/Debug
rm -Rvf ws2server/Release
rm -Rvf ws2server/STANDARDSDK_500\ \(x86\)
rm -Rvf ws2server/Win32
rm -Rvf ws2server/ws2server.vcproj.DESKTOP-LGKFQ39.jasper.user

